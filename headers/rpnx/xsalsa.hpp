...
namespace rpnx::djb_crypto
{
    ...
    void stream_xsalsa(InputIt input, InputIt inputEnd, OutputIt output, NonceIt nonce, KeyIt key, std::uint64_t stream_position = 0)
    {
        std::array<std::byte, 32> subkey;
        rpnx::djb_crypto::core_hsalsa<Rounds>(subkey.data(), nonce, key);
        rpnx::djb_crypto::stream_salsa_xor_ic<Rounds>(output, input, std::distance(input, inputEnd), nonce + 16, stream_position, subkey.data());
    }
}
